...
//      "current "+name+" in \n"+this.freshRecords.keySet();
    ...
    Seq.of(record, "\n", k.meths(), m->visitMeth(record, m, MethExprKind.Kind.Unreachable, leastSpecific));
    record.append("}\n");
  ...
  public void visitMeth(StringBuilder buffer, MIR.Meth meth, MethExprKind kind, Map<Id.MethName, MIR.Sig> leastSpecific) {
    ...
    buffer.append(") {\n");
    switch (kind) {
      case MethExprKind.Kind kind_ -> writeMethBody(buffer, meth, kind_, mustCast);
  ...
  private void writeMethBody(StringBuilder buffer, MIR.Meth meth, MethExprKind.Kind kind, boolean mustCast) {
    switch (kind) {
